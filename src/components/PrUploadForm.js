...
import { updatePr } from '../api'; // Ensure this function is correctly set up to handle both actions
...
const PrUploadForm = () => {
  ...
  const handleUpdatePR = async () => {
    ...
    if(!token) return;
    try {
      const data = await updatePr(newPrNumber, token); // Call the upload function
      setMessage(data.message);
